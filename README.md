...
Copy `.env.example` to `.env`; Docker Compose loads it automatically. Set both secrets
to long, random values before exposing the stack beyond localhost.

| Variable | Default | Description |
|---|---|---|
| `JWT_SECRET` | *(required; 32+ random characters)* | Signs session cookies. The API refuses missing, weak, or placeholder values. |
| `POSTGRES_PASSWORD` | *(required)* | Password for the local Postgres service. Keep it out of source control. |
...
- **Set strong secrets.** `JWT_SECRET` and `POSTGRES_PASSWORD` are required by Compose;
  the API refuses weak or missing JWT secrets, and Postgres is bound to localhost by
  default rather than exposed on all host interfaces.
